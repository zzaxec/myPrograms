...
public class NPanel extends JPanel{
	//Components
	JPanel defaultPanel = new JPanel(), randomPanel = new JPanel();
	JButton consonant = new JButton("K"), vocal = new JButton("V"), clear = new JButton("Clear"), start = new JButton("Generate"), randomStart = new JButton("Generate Random");
	...
	private class RandomListener implements ActionListener{
		...
		public void actionPerformed( ActionEvent e ) {
			if(e.getSource().equals(randomStart)){
				nicknameField.setText("Your random Nickname is :"+"\n"+g.nextRandomNick(comboBox.getItemAt(comboBox.getSelectedIndex())));
			}
		}
